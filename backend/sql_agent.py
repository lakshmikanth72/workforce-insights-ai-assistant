...
def _query_for(question: str) -> Optional[Tuple[str, str]]:
    ...
    # 1. High-risk employees by department
    ...
    if (
        ("highest attrition" in text or "highest turnover" in text)
        and "department" in text
    ):
        return (
            "SELECT department, "
            "COUNT(*) FILTER (WHERE LOWER(attrition_flag::text) IN "
            "('1', 'true', 'yes') OR LOWER(attrition::text) = 'yes') "
            "AS attrition_count, "
            "COUNT(*) AS total_employees, "
            "ROUND(100.0 * COUNT(*) FILTER (WHERE "
            "LOWER(attrition_flag::text) IN ('1', 'true', 'yes') "
            "OR LOWER(attrition::text) = 'yes') / NULLIF(COUNT(*), 0), 2) "
            "AS attrition_rate "
    ...
    # 4. Employees who actually left
    if (
        "actually left" in text
        or "have left" in text
        or "who left" in text
        or "actual attrition" in text
        or "how many left" in text
    ):
        return (
            "SELECT COUNT(*) FILTER (WHERE "
            "LOWER(attrition_flag::text) IN ('1', 'true', 'yes') "
            "OR LOWER(attrition::text) = 'yes') "
            "AS actual_attrition_count "
            "FROM workforce_ai_view;",
    ...
    if (
        "predicted to leave" in text
        or "predicted attrition" in text
        or "predicted to quit" in text
    ):
        return (
            "SELECT COUNT(*) AS predicted_to_leave_count "
            "FROM employee_attrition_predictions "
            "WHERE LOWER(predicted_attrition::text) IN "
            "('1', 'true', 'yes') "
            "OR LOWER(predicted_attrition_label::text) "
            "IN ('yes', 'leave', 'left');",
    ...
    if "average" in text and "satisfaction" in text:
        return (
            "SELECT ROUND(AVG(job_satisfaction)::numeric, 2) "
    ...
    for dept in ("sales", "research & development", "human resources"):
        if dept in text and (
            ...
            return (
                ...
                f"FROM workforce_ai_view "
                f"WHERE LOWER(department) = '{dept}';",
...
def _format_explanation(
    ...
    # Multiple rows, such as department breakdown
    items = []
...
def answer_sql_question(
    ...
    if not normalized_sql.startswith("SELECT") or any(
        ...
        return {
            "sql": sql,
            "rows": [],
            "explanation": "Only safe read-only queries are permitted.",
        }
    ...
    try:
        ...
        return {
            "sql": sql,
            "rows": rows,
            "explanation": _format_explanation(label, rows),
        }

    except Exception as error:
        # Print the REAL PostgreSQL error to Render logs
        print(
            f"POSTGRESQL ERROR: {type(error).__name__}: {error}",
            flush=True,
        )
        ...
        return {
            ...
            "rows": [],
            "explanation": (
                f"Unable to query PostgreSQL ({label}): "
                f"{error}. Please verify database connection and views."
            ),
        }
